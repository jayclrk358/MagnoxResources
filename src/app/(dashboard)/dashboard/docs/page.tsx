    ...
    <div>
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-dark-600 bg-dark-800 p-6">
            ...
            <p className="mb-4 text-sm text-gray-400">
              ...
              your server. Use <code className="text-gray-300">%placeholder%</code>{" "}
      ...
      {activeTab === "aurolobby" && (
        <div className="space-y-4">
          ...
          <h3 className="text-sm font-semibold text-gray-300">Configuration Files</h3>
          ...
          <ConfigFileCard
            ...
            keys={["spawn.world", "spawn.x", "spawn.y", "spawn.z", "spawn.yaw", "spawn.pitch", "teleport-on-join", "clear-inventory-on-join"]}
          />
          <ConfigFileCard
            name="Protection"
            fileName="configs/protection.yml"
            description="World protection toggles for the lobby."
            keys={["no-pvp", "no-hunger", "no-build", "no-item-drop", "no-item-pickup", "no-fall-damage", "no-all-damage", "void-protection", "no-weather", "no-mob-spawning", "no-fire-spread", "no-block-interact", "no-leaf-decay", "no-explosions", "disable-achievements"]}
          />
          <ConfigFileCard
            name="Double Jump"
            fileName="configs/doublejump.yml"
            description="Double jump mechanics and effects."
            keys={["enabled", "horizontal-multiplier", "vertical-force", "cooldown-seconds", "sound", "sound-volume", "sound-pitch", "particles", "particle-type", "particle-count"]}
          />
          <ConfigFileCard
            name="Scoreboard"
          ...
          <ConfigFileCard
            ...
            keys={["enabled", "update-interval-ticks", "header", "footer", "name-format"]}
          />
          <ConfigFileCard
            name="Hotbar"
            fileName="configs/hotbar.yml"
            description="Items given to players on join with click actions."
            keys={["enabled", "items.{id}.material", "items.{id}.slot", "items.{id}.name", "items.{id}.lore", "items.{id}.action"]}
          />
          <ConfigFileCard
            name="Server Selector"
            fileName="configs/server-selector.yml"
            description="GUI menu for BungeeCord server switching."
            keys={["title", "size", "border", "servers.{id}.material", "servers.{id}.name", "servers.{id}.slot", "servers.{id}.bungee-name"]}
          />
          <ConfigFileCard
            name="Cosmetics"
            fileName="configs/cosmetics.yml"
            description="Particle trails, chat colors, join/leave messages, nickname colors, and gadgets."
            keys={["particle-trails.{id}", "chat-colors.{id}", "join-messages.{id}", "leave-messages.{id}", "nickname-colors.{id}", "gadgets.{id}"]}
          />
          <ConfigFileCard
            name="Player"
            fileName="configs/player.yml"
            description="Player state on join (heal, feed, gamemode, speed)."
            keys={["heal-on-join", "feed-on-join", "clear-effects-on-join", "gamemode-on-join", "fly-speed", "walk-speed"]}
          />
          <ConfigFileCard
            name="Chat"
          ...
          <ConfigFileCard
            ...
            keys={["custom-join-message", "join-message", "leave-message", "join-sound", "join-sound-volume", "join-sound-pitch"]}
          />
          <ConfigFileCard
            name="Messages"
      ...
      {activeTab === "auropunish" && (
        <div className="space-y-4">
          <div className="rounded-xl border border-dark-600 bg-dark-800 p-6">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {["Permanent Bans", "Temp Bans", "Permanent Mutes", "Temp Mutes", "Kicks", "Warnings", "History", "MySQL", "MariaDB", "PostgreSQL", "SQLite"].map((f) => (
                <span key={f} className="rounded-full bg-dark-600 px-3 py-1 text-xs text-gray-300">
                  {f}
                </span>
              ))}
          ...
          <div className="rounded-xl border border-dark-600 bg-dark-800 p-6">
            <h3 className="mb-2 text-sm font-semibold text-white">Commands</h3>
            <div className="space-y-2">
              <Placeholder name="/ban <player> <reason>" description="Permanently ban a player" />
              <Placeholder name="/tempban <player> <duration> <reason>" description="Temporarily ban a player (e.g. 1d2h30m)" />
              <Placeholder name="/unban <player>" description="Remove an active ban" />
              <Placeholder name="/mute <player> <reason>" description="Permanently mute a player" />
              <Placeholder name="/tempmute <player> <duration> <reason>" description="Temporarily mute a player" />
              <Placeholder name="/unmute <player>" description="Remove an active mute" />
              <Placeholder name="/kick <player> <reason>" description="Kick a player from the proxy" />
              <Placeholder name="/warn <player> <reason>" description="Issue a warning to a player" />
              <Placeholder name="/history <player>" description="View a player's full punishment history" />
            </div>
          </div>
          ...
          <div className="rounded-xl border border-dark-600 bg-dark-800 p-6">
            <h3 className="mb-2 text-sm font-semibold text-white">Permissions</h3>
            <div className="space-y-2">
              <Placeholder name="punishments.ban" description="Use /ban command" />
              <Placeholder name="punishments.tempban" description="Use /tempban command" />
              <Placeholder name="punishments.unban" description="Use /unban command" />
              <Placeholder name="punishments.mute" description="Use /mute command" />
              <Placeholder name="punishments.tempmute" description="Use /tempmute command" />
              <Placeholder name="punishments.unmute" description="Use /unmute command" />
              <Placeholder name="punishments.kick" description="Use /kick command" />
              <Placeholder name="punishments.warn" description="Use /warn command" />
              <Placeholder name="punishments.history" description="View punishment history" />
            </div>
          </div>
          ...
          <div className="rounded-xl border border-dark-600 bg-dark-800 p-6">
            ...
            <p className="mb-3 text-sm text-gray-400">
              Available in message templates in <code className="text-gray-300">config.toml</code>.
            </p>
            <div className="space-y-2">
              ...
              <Placeholder name="{type}" description="Punishment type (BAN, MUTE, etc.)" />
      ...
      {activeTab === "api" && (
        <div className="space-y-4">
          <div className="rounded-xl border border-dark-600 bg-dark-800 p-6">
            ...
            <p className="text-sm text-gray-400">
              REST API endpoints used by plugins to communicate with the panel.
              All requests require a <code className="text-gray-300">Bearer</code>{" "}
              token in the <code className="text-gray-300">Authorization</code> header.
            </p>
          </div>

          <Collapsible title="POST /api/v1/sync — Push configs to panel" defaultOpen>
            <p className="mb-3 text-sm text-gray-400">
              Called by the plugin on startup. Registers the plugin and uploads
              all config files. Auto-creates the server entry if the token is
              new.
            </p>
  ...
  "configs": [
    {
      ...
      "content": { "prefix": "&6[AuroLobby] &r" }
          ...
          <Collapsible title="GET /api/v1/sync — Poll for pending changes">
            <p className="mb-3 text-sm text-gray-400">
              Called periodically by the plugin. Returns any config changes made
              through the panel that haven't been applied yet.
            </p>
            <div className="rounded-lg bg-dark-900 p-4">
  ...
  "changes": [
    {
      ...
      "content": { "no-pvp": true, ... }
          ...
          <Collapsible title="POST /api/v1/verify — Validate token">
            <p className="mb-3 text-sm text-gray-400">
              Verify a server token and retrieve server info. Returns the server
              ID, name, and list of registered plugins.
            </p>
            <div className="rounded-lg bg-dark-900 p-4">
              <span className="text-xs font-medium text-gray-400">Response</span>
              <pre className="mt-1 overflow-x-auto text-xs text-gray-300">{`{
